...
void loop()
{
  ...
  while (1) {
    ...
    if (tempval != val2) {
      ...
      errorSum += .000619; //value calculated to prevent incrementing

    if(flip) {  
      BTserial.print("Pitch = ");
      changePitch = pitch*90.00/5.50; //change pitch to be correct units since it gets altered when adjusting to use gyro as well
      BTserial.println(changePitch - errorSum); //subtract the running error to correct incrementing
    }
    flip = false;
